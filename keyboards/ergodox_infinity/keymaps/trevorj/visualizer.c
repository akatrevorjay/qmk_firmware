...
bool led_backlight_keyframe_one_period_sweep(keyframe_animation_t *animation,
                                             visualizer_state_t *state);
bool led_backlight_keyframe_half_period_sweep_to_on(
    keyframe_animation_t *animation, visualizer_state_t *state);
bool led_backlight_keyframe_half_period_sweep_to_off(
    keyframe_animation_t *animation, visualizer_state_t *state);
    ...
    .frame_lengths =
        {
            ...
            gfxMillisecondsToTicks(
                GRADIENT_TIME), // left_to_right (mirrored, so inside out)
            0,                  // normal leds
            gfxMillisecondsToTicks(GRADIENT_TIME), // left to rigt (outside in)
            0,                                     // mirror leds
            gfxMillisecondsToTicks(
...
// Don't change anything else than state->target_lcd_color and state->layer_text
// as that's the only thing that the simple_visualizer assumes that you are
// updating Also make sure that the buffer passed to state->layer_text remains
// valid until the previous animation is stopped. This can be done by either
// double buffering it or by using constant strings
